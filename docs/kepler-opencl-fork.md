...
- Flash attention (`GGML_OP_FLASH_ATTN_EXT`): `supports_op` requires a non-null per-`(dk,dv)` kernel after `load_cl_kernels` (FP16 flash variants are not built without `cl_khr_fp16`). Init never calls `clCreateKernel` on a null program after a failed relaxed build.
